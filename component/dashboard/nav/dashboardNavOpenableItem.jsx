...
export default function DashboardNavOpenableItem({name, icon, link, links, children}) {
        ...
        <li>
            <div className={'flex items-center justify-between gap-[10px] duration-500 transition-all data-[opened="true"]:bg-white/30 rounded-t-[10px]'} data-opened={isOpened}>
                <Link href={link} className={'w-[90%]'}>
                    <button data-active={(links.includes(pathName))} tabIndex={-1} className={'flex items-center gap-[10px] py-[8px] px-[10px] rounded-[10px] data-[active="true"]:bg-white/20 hover:bg-white/20 w-full transition-all duration-500 relative data-[active="true"]:before:absolute data-[active="true"]:before:h-full data-[active="true"]:before:w-[5px] data-[active="true"]:before:bg-theme data-[active="true"]:before:left-full data-[active="true"]:before:top-0 data-[active="true"]:before:translate-x-[15px] data-[active="true"]:before:rounded-l-[20px]'}>
